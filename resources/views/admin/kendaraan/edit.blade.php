...
<div class="max-w-3xl fade-in">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-headline-md text-on-surface">Edit Kendaraan</h1>
    </div>

    <div class="bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant/20 p-6">
        <form method="POST" action="{{ route('admin.kendaraan.update', $kendaraan) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="plat_nomor" value="{{ old('plat_nomor', $kendaraan->plat_nomor) }}" required
                        class="w-full px-4 py-2.5 border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 rounded-xl text-body-md font-mono bg-surface-container-lowest outline-none transition">
                ...
                <div>
                    ...
                    <select name="jenis" required class="w-full px-4 py-2.5 border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 rounded-xl text-body-md bg-surface-container-lowest outline-none transition">
                        @foreach(\App\Enums\JenisKendaraan::cases() as $jenis)
                            <option value="{{ $jenis->value }}" {{ old('jenis', $kendaraan->jenis->value) === $jenis->value ? 'selected' : '' }}>{{ $jenis->label() }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="tahun" value="{{ old('tahun', $kendaraan->tahun) }}" required min="1990" max="{{ date('Y') + 1 }}"
                        class="w-full px-4 py-2.5 border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 rounded-xl text-body-md bg-surface-container-lowest outline-none transition">
                ...
                <div>
                    ...
                    <input type="number" name="kapasitas" value="{{ old('kapasitas', $kendaraan->kapasitas) }}" required min="1" max="60"
                        class="w-full px-4 py-2.5 border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 rounded-xl text-body-md bg-surface-container-lowest outline-none transition">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="harga_sewa_per_hari" value="{{ old('harga_sewa_per_hari', $kendaraan->harga_sewa_per_hari) }}" required min="0" step="1000"
                        class="w-full px-4 py-2.5 border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 rounded-xl text-body-md bg-surface-container-lowest outline-none transition">
                ...
                <div>
                    ...
                    <select name="status" required class="w-full px-4 py-2.5 border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 rounded-xl text-body-md bg-surface-container-lowest outline-none transition">
                        @foreach(\App\Enums\StatusKendaraan::cases() as $status)
                            <option value="{{ $status->value }}" {{ old('status', $kendaraan->status->value) === $status->value ? 'selected' : '' }}>{{ $status->label() }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                @if($kendaraan->gambar)
                    <div class="mb-3">
                        <img src="{{ asset('storage/uploads/kendaraan/' . $kendaraan->gambar) }}" alt="{{ $kendaraan->nama_kendaraan }}" class="w-32 h-24 object-cover rounded-xl border border-outline-variant/20">
                        <p class="text-xs text-on-surface-variant mt-1">Gambar saat ini. Upload baru untuk mengganti.</p>
                    </div>
                @endif
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" required class="w-full px-4 py-2.5 border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 rounded-xl text-body-md bg-surface-container-lowest outline-none transition">{{ old('deskripsi', $kendaraan->deskripsi) }}</textarea>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('admin.kendaraan.index') }}" class="px-6 py-2.5 bg-surface-container text-on-surface-variant text-label-md rounded-xl hover:bg-surface-variant transition-all">Batal</a>
            </div>
        </form>
    </div>
</div>
