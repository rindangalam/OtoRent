...
@section('content')
    <div class="mb-6">
        <a href="{{ route('kendaraan.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-primary-500 transition">
    ...
    <form method="POST" action="{{ route('booking.store') }}" class="max-w-3xl">
        ...
        <div x-data="{
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="flex items-center justify-between p-3 bg-primary-50 rounded-lg">
                        <span class="text-sm text-primary-700">Harga sewa</span>
                        <span class="text-lg font-bold text-primary-600">Rp {{ number_format($kendaraan->harga_sewa_per_hari, 0, ',', '.') }}<span class="text-sm font-normal text-primary-500"> / hari</span></span>
                    </div>

                    {{-- Tipe Sewa --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-800 mb-3">Tipe Sewa</label>
                        <div class="grid grid-cols-2 gap-3">
                            <label class="relative cursor-pointer">
                                <input type="radio" name="tipe_sewa" value="driver" x-model="tipeSewa" class="peer sr-only">
                                <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-gray-200 peer-checked:border-primary-500 peer-checked:bg-primary-50 transition-all text-center hover:border-gray-300">
                                    <svg class="w-8 h-8 text-gray-400 peer-checked:text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                    </svg>
                                    <span class="text-sm font-medium text-gray-700 peer-checked:text-primary-700">Pakai Driver</span>
                                </div>
                            </label>
                            <label class="relative cursor-pointer">
                                ...
                                <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-gray-200 peer-checked:border-primary-500 peer-checked:bg-primary-50 transition-all text-center hover:border-gray-300">
                                    ...
                                    <span class="text-sm font-medium text-gray-700 peer-checked:text-primary-700">Lepas Kunci</span>
                                </div>
                            </label>
                        </div>
                        @error('tipe_sewa')
                    ...
                    <div x-show="tipeSewa === 'driver'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select id="driver_id" name="driver_id"
                                ...
                                class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:ring-opacity-50 text-sm">
                        ...
                        <div>
                            <label for="lokasi_jemput_driver" class="block text-sm font-medium text-gray-700 mb-1">Lokasi Jemput</label>
                            <input type="text" id="lokasi_jemput_driver" name="lokasi_jemput" value="{{ old('lokasi_jemput') }}" placeholder="Alamat penjemputan"
                                class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:ring-opacity-50 text-sm" required>
                            @error('lokasi_jemput')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="lokasi_tujuan" class="block text-sm font-medium text-gray-700 mb-1">Lokasi Tujuan <span class="text-gray-400">(opsional)</span></label>
                    ...
                    <div x-show="tipeSewa === 'lepas_kunci'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-800 mb-3">Cara Mendapatkan Kendaraan</label>
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative cursor-pointer">
                                    <input type="radio" name="metode_antar" value="diantar" x-model="metodeAntar" class="peer sr-only">
                                    <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-gray-200 peer-checked:border-accent-500 peer-checked:bg-amber-50 transition-all text-center hover:border-gray-300">
                                        <svg class="w-8 h-8 text-gray-400 peer-checked:text-accent-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                                        </svg>
                                        <span class="text-sm font-medium text-gray-700">Mobil Diantar</span>
                                    </div>
                                </label>
                                <label class="relative cursor-pointer">
                                    ...
                                    <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-gray-200 peer-checked:border-accent-500 peer-checked:bg-amber-50 transition-all text-center hover:border-gray-300">
                                        ...
                                        <span class="text-sm font-medium text-gray-700">Jemput Sendiri</span>
                                    </div>
                                </label>
                            </div>
                            @error('metode_antar')
                        ...
                        <div x-show="metodeAntar === 'diantar'" x-transition class="space-y-4">
                            <div>
                                <label for="lokasi_jemput_diantar" class="block text-sm font-medium text-gray-700 mb-1">Lokasi Pengantaran</label>
                            ...
                            <div>
                                <label for="ongkos_antar" class="block text-sm font-medium text-gray-700 mb-1">Ongkos Antar</label>
                                <div class="relative">
                                    ...
                                    <input type="number" id="ongkos_antar" name="ongkos_antar" x-model.number="ongkosAntar" min="0" step="1000" placeholder="0"
                                        class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:ring-opacity-50 text-sm pl-10" required>
                                </div>
                                <p class="text-xs text-gray-400 mt-1">Masukkan ongkos antar sesuai kesepakatan</p>
                                @error('ongkos_antar')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div x-show="metodeAntar === 'jemput_sendiri'" x-transition class="space-y-4">
                            <div class="flex items-start gap-3 p-4 bg-amber-50 rounded-lg border border-amber-200">
                                ...
                                <p class="text-sm text-amber-800">Silakan datang ke lokasi rental untuk mengambil kendaraan yang telah disiapkan.</p>
                            </div>
                            <div>
                                <label for="lokasi_jemput_jemput" class="block text-sm font-medium text-gray-700 mb-1">Alamat Lokasi Rental</label>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 class="text-sm font-semibold text-gray-800 mb-4">Tanggal Sewa</h3>
            ...
            {{-- Ringkasan Biaya --}}
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 class="text-sm font-semibold text-gray-800 mb-4">Ringkasan Biaya</h3>
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">
                            Kendaraan: <span x-text="durasi"></span> hari &times; Rp <span x-text="hargaPerHari.toLocaleString('id-ID')"></span>
                        </span>
                        <span class="font-medium text-gray-900" x-text="formatRp(totalKendaraan)"></span>
                    </div>
                    <template x-if="tipeSewa === 'driver' && tarifDriver > 0">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-medium text-gray-900" x-text="formatRp(totalDriver)"></span>
                        </div>
                    </template>
                    <template x-if="metodeAntar === 'diantar' && ongkosAntar > 0">
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">Ongkos Antar</span>
                            <span class="font-medium text-gray-900" x-text="formatRp(ongkosAntar)"></span>
                        </div>
                    </template>
                    <div class="border-t border-gray-200 pt-3 flex justify-between text-base">
                        ...
                        <span class="font-bold text-primary-500 text-lg" x-text="formatRp(grandTotal)"></span>
                    </div>
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('kendaraan.index') }}" class="text-sm text-gray-500 hover:text-gray-700 transition">Batal</a>
            </div>
        </div>
    </form>
@endsection
